...
fun isNumberHappy(number: Int): Boolean =
    ((number / 10 % 10) + (number % 10)) == ((number / 10 / 10 % 10) + (number / 10 / 10 / 10 % 10))
...
fun circleInside(
    ...
    (sqr(x1 - x2) + sqr(y1 - y2) <= sqr(r2)) && r2 - r1 >= 0
...
fun brickPasses(a: Int, b: Int, c: Int, r: Int, s: Int): Boolean =
    minOf(a, b, c) <= min(s, r) && a + b + c - minOf(a, b, c) - maxOf(a, b, c) <= s + r - min(s, r)
